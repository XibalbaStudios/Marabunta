...
-- @param dim Slot size.
-- @return Slot index.
function FitToSlot (value, base, dim)
...
-- @return Index.
-- @see IndexToCell
function CellToIndex (col, row, w)
	return (row - 1) * w + col
end
...
GridIter = InstancedAutocacher(function()
...
function IndexInRange (index, size, okay_after)
...
-- Cache some routines.
_BoxesIntersect_ = BoxesIntersect
_CellToIndex_ = CellToIndex
_DivRem_ = DivRem
_SortPairs_ = SortPairs
_SwapIf_ = SwapIf